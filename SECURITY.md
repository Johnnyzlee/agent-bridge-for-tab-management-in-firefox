...
Firefox's Native Messaging channel does not transmit the calling extension's ID to the host process, so the host cannot inspect the caller at runtime. The authorization boundary is enforced by Firefox against the host manifest's `allowed_extensions`, and the host independently verifies that its own registration still authorizes exactly the expected extension ID before serving the configuration (defense in depth).
