...
No generated JavaScript is included in the v0.3.1 AMO source package. Install the locked dependencies and build from its root:
...
The reviewable extension is generated in `dist/firefox-extension/`. `extension/background.ts` is bundled by `esbuild`; `extension/manifest.json`, `options.html`, `options.css`, and `options.js` are copied without transformation. Source maps are included.
...
The server must remain running for the extension to show a connected state. Version 0.3.1 permits only one server instance on the configured port.
